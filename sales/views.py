...
import pytz
from hrm_system import serializers
from hrm_system.serializers import *
...
class InvoiceAPIView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if "products" in request.data and isinstance(request.data["products"], list):
            for product_data in request.data["products"]:
                if "product" in product_data and "quantity" in product_data and "price" in product_data:
                    if isinstance(product_data["product"], (int, float)) and isinstance(product_data["quantity"], (int, float)):
                        if product_data["product"] > 0 and product_data["quantity"] > 0:
                            ...
                            try:
                                ...
                                return Response({'data': 'invoice has been created'}, status=status.HTTP_201_CREATED)
                        ...
                        return Response({"message": "Product, price and number must be number."}, status=status.HTTP_400_BAD_REQUEST)

                else:
                    # Handle missing or invalid data
                    return Response({'error': 'product, quantity and price is required'}, status=status.HTTP_400_BAD_REQUEST)
        else:
            # Handle missing or invalid data
            print('i am here')
    ...
    def get(self, request, pk=None):
        ...
        if pk is not None:
            ...
            InvoiceProduct = self.get_invoice_product(
                invoice_serializer.data['id'])
        ...
        if serializer.is_valid():
            ...
            if products_data:
                ...
                if invoice_product_serializer.is_valid():
                    ...
                    return Response(invoice_product_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
